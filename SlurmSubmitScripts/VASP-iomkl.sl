#!/bin/bash
# Scalability tests 
# for i in {4..12}; do sbatch --ntasks=$(($i*$i)) --nodes=$((($i*$i)/12))-$((1+($i*$i)/12)) -C wm /share/SubmitScripts/slurm/VASP-iomkl.sl ; done
#SBATCH -J VASPBench
#SBATCH -A nesi99999
#SBATCH --time=1:59:00
##SBATCH --ntasks=96
...
cp -pr /share/test/VASP/$TEST/input/* .
...
mkdir -p /share/test/VASP/OUT/$TEST
rm WAVECAR CHG*
cp -pr $SCRATCH_DIR /share/test/VASP/OUT/$TEST/
TEMPS=$(cat OUTCAR | grep "Total CPU time used" | awk '{print $(NF)}')
echo "$SLURM_NPROCS   $TEMPS" >> /share/test/VASP/benchmark-$TEST-$VASPBIN-5.3.5-iomkl-4.6.13-$LMOD_SYSTEM_NAME.log
